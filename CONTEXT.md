...
This repository is a single-process GitLab AI review service for self-hosted GitLab. It receives Merge Request webhook events, schedules in-process asynchronous handling, re-fetches the authoritative MR state from GitLab, runs deterministic validation checks, optionally calls provider-backed AI review logic, and then updates the AI-managed section of the MR description and/or posts a review note.

The design is intentionally centralized and operationally small: one FastAPI service, no separate worker process, no durable SQLite queue, and provider-neutral AI orchestration.
...
- FastAPI API for webhook intake
- in-process async task for MR processing
...
3. Validated events are accepted and scheduled as an in-process async task.
4. The task re-fetches the current MR state from GitLab before processing.
...
9. Structured logs and metrics record webhook, processing, and review outcomes.
...
- `app/api/webhook.py` — signature validation, timestamp validation, event relevance checks, dedupe handling, and in-process scheduling
...
- Events are deduplicated in memory while a task is pending or processing.
- The current runtime model does not rely on a durable queue or lease-based worker recovery.
- If the process exits, in-flight work is not recovered automatically.
...
- Preserve the existing single-process architecture unless the user explicitly asks for a different design.
...
- Keep the runtime single-process and avoid introducing a queue or second service.

## 10. Current project status

This repository is already beyond the initial bootstrap phase. It includes single-process webhook handling, security validation, AI review orchestration, deterministic evaluation, operational observability, and deployment-hardening documentation.

The active deployment model is a single-server Linux setup with a single FastAPI service and no separate worker. Deployment files are documented under [deploy/systemd](deploy/systemd) and the production deployment guide is under [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).

The immediate priority is to preserve this working implementation and document the exact service layout, filesystem permissions, and Linux verification steps needed before claiming production readiness.

## 11. Phase deployment architecture
...
- one FastAPI service handles HTTP/webhook intake, validation, in-process MR processing, AI review, and GitLab updates
...
This phase documents deployment readiness for the single-process model. Actual Linux runtime behavior must still be verified on the target host.

## 12. Deployment artifacts in this repo

- [deploy/systemd/git-ai-agent.service](deploy/systemd/git-ai-agent.service) — single-process application service template
- [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) — Linux deployment procedures, permissions, startup order, and smoke test guidance
...
- single-process webhook architecture validation
...
- deterministic verification of the in-process MR processing path
- observability-focused verification for health, readiness, and metrics

### NOT IMPLEMENTED

- Phase 8 live AI evaluation
- new providers or architecture changes
- any restoration of the removed queue or worker process

The current validation covers the single-process architecture and the offline failure/recovery checks actually executed in this workspace. Live provider testing remains pending and must remain gated behind real credentials.
